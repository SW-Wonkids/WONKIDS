<template>
  ...
  <div id="app" style="margin-left: 55px; margin-right: 55px;font-family: 'NanumBarunGothicYetHangul';">
    ...
    <hr>
    <div v-if="showQuiz" style="text-align: center; font-size: 300%;" class="card">
    ...
    <div class="container">
      <div v-if="showResult" style="text-align: center;" class="card">
        <h2>결과: {{ result }}</h2>

        <div v-if="result === '꼬부기'">물 타입 포켓몬인 꼬부기는 잔잔하게 흐르는 물의 특징을 침착하고 차분한 성향으로 연관지어 가정했습니다.<br> 목돈을 한꺼번에 맡기고 차분히 기다릴 수 있는 성향임을 가정하여 예금 4개를 추천합니다.</div>
        <div v-if="result === '파이리'">불꽃 타입 포켓몬인 파이리는 활활 타오르는 불의 특징을 활발하고 화끈한 성격으로 연관지어 가정했습니다.<br> 꾸준히 일정 금액을 넣으며 만기일을 기다리는 성향임을 가정하여 적금 4개를 추천합니다.</div>
        <div v-if="result === '피카츄'">전기 타입 포켓몬인 피카츄는 파이리와 꼬부기를 선택한 사람의 중간으로 가정하여, 적금 2개, 예금 2개를 추천합니다.</div>
...
const answersA = ref([
  { text: "A. 나는 농사를 지으며 동물들과 함께 지구에서 살아갈 것이다 " },
